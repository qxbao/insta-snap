...
import { SnapshotCron } from "../stores/app.store";
...
export async function getFullFollowersList(
  ...
  const followers = new Set(checkpointRecord.followers.add || []);
...
export async function getFullFollowingList(
  ...
  const following = new Set(checkpointRecord.following.add || []);
  const deltasToApply = upToTimestamp
    ? meta.logTimeline.filter(
        (ts) => ts > latestCheckpoint && ts <= upToTimestamp,
      )
...
export async function saveFollowersSnapshot(
  targetId: string,
  currentFollowers: Node[],
  logger?: Logger,
): Promise<void> {
  const timestamp = Date.now();
  const followerIds = currentFollowers.map((u) => u.id);

  await updateGlobalUserMap(currentFollowers);
  await saveSnapshotInternal(targetId, timestamp, followerIds, null, logger);
}
...
export async function getFollowersHistory(
  ...
  logger?: Logger,
	...
	logger?.info(`Fetching followers history for user ${targetId}`);
  const meta = await getUserSnapshotMeta(targetId);
  if (!meta) {
    return [];
  }
...
export async function saveFollowingSnapshot(
  targetId: string,
  currentFollowing: Node[],
  logger?: Logger,
): Promise<void> {
  const timestamp = Date.now();
  const followingIds = currentFollowing.map((u) => u.id);
  await updateGlobalUserMap(currentFollowing);
  await saveSnapshotInternal(targetId, timestamp, null, followingIds, logger);
}
...
> {
  logger?.info(`Fetching following history for user ${targetId}`);
  const meta = await getUserSnapshotMeta(targetId);
  if (!meta) {
    return [];
  }
...
export async function getUserSnapshotCount(targetId: string): Promise<number> {
  ...
  for (const metaKey of metaKeys) {
    ...
    if (userInfo && meta) {
      trackedUsers.push({
        ...
        lastSnapshot:
          meta.logTimeline.length > 0 ? Math.max(...meta.logTimeline) : null,
