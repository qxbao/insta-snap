...
export class BackgroundService {
	...
	async checkSecurityConfig(): Promise<void> {
		try {
			...
			const [localData, existingConfigs] = await Promise.all([
				...
				Promise.all(
					keysToMigrate.map((key) => database.internalConfig.get(key)),
				),
			]);
			...
			for (let i = 0; i < keysToMigrate.length; i++) {
				...
				if (!existing && val && typeof val === "string") {
					...
					migratePromises.push(
						database.writeEncryptedConfig(key, val, this.encryptor),
					);
	...
	initializeAlarms(): void {
		...
		this.logger.info(
			"Snapshot subscription alarm initialized (30min interval)",
		);
	}
		...
		try {
			...
			for (const cron of crons) {
				...
				if (now - cron.lastRun >= cron.interval * 60 * 60 * 1000) {
					...
					// Jitter between cron executions
	...
	private routeMessage(
		...
		switch (message.type) {
			...
			case ActionType.SAVE_SNAPSHOT:
				isAsync = true;
				const { userId, timestamp, followerIds, followingIds } =
					message.payload;
				database
