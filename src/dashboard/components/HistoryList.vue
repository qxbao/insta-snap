...
const loadSnapshotDetails = async (timestamp: number) => {
	...
	try {
		...
		if (record) {
			const fieldData = props.type === "followers" ? record.followers : record.following;
			snapshotDetails.value.set(timestamp, {
				added: fieldData.add || [],
				removed: fieldData.rem || [],
				userMap,
			});
		}
	} catch (err) {
		...
		<div class="space-y-2">
			...
			>
				...
				>
					...
					>
						<div
							v-if="snapshotDetails.get(entry.timestamp)!.added.length > 0"
						>
							<h4 class="text-sm font-semibold text-green-600 dark:text-green-400 mb-2">
								{{ type === "followers" ? "Followers" : "Following" }} Added ({{ snapshotDetails.get(entry.timestamp)!.added.length }})
							</h4>
							...
							<div
								v-if="getVisibleCount(entry.timestamp, 'added') < snapshotDetails.get(entry.timestamp)!.added.length"
								class="flex gap-2 mt-2"
							>
								<button
									@click="loadMore(entry.timestamp, 'added')"
								...
								<button
									@click="loadAll(entry.timestamp, 'added', snapshotDetails.get(entry.timestamp)!.added.length)"
									class="text-sm text-emerald-600 dark:text-emerald-400 hover:underline"
								>
									Load all
								</button>
						...
						<div
							v-if="snapshotDetails.get(entry.timestamp)!.removed.length > 0"
						>
							<h4 class="text-sm font-semibold text-red-600 dark:text-red-400 mb-2">
								{{ type === "followers" ? "Followers" : "Following" }} Removed ({{ snapshotDetails.get(entry.timestamp)!.removed.length }})
							</h4>
							<div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
								<div
							...
							<div
								v-if="getVisibleCount(entry.timestamp, 'removed') < snapshotDetails.get(entry.timestamp)!.removed.length"
								class="flex gap-2 mt-2"
							>
								<button
									@click="loadMore(entry.timestamp, 'removed')"
									class="text-sm text-emerald-600 dark:text-emerald-400 hover:underline"
								>
									Load more
								</button>
								<span class="text-gray-400">•</span>
								<button
									@click="loadAll(entry.timestamp, 'removed', snapshotDetails.get(entry.timestamp)!.removed.length)"
									class="text-sm text-emerald-600 dark:text-emerald-400 hover:underline"
								>
									Load all
								</button>
						...
						<div
							v-if="
								snapshotDetails.get(entry.timestamp)!.added.length === 0 &&
								snapshotDetails.get(entry.timestamp)!.removed.length === 0
							"
