    ...
    <div class="space-y-2">
      <div
        ...
        :class="[
          ...
          isAffectedByDelete(entry.timestamp)
            ? 'border-red-400 dark:border-red-500 bg-red-50/50 dark:bg-red-900/20 shadow-lg'
            : 'border-lighter/40',
        ]"
      >
        <div
        ...
        >
          <div class="flex items-center gap-4 flex-1">
            <button class="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
              <Fa6SolidChevronDown v-if="expandedItems.has(entry.timestamp)" class="w-4 h-4" />
              <Fa6SolidChevronRight v-else class="w-4 h-4" />
            </button>
            <div
              :class="['w-3 h-3 rounded-full', entry.isCheckpoint ? 'bg-blue-500' : 'bg-gray-400']"
            ></div>
            <div>
            ...
            <p v-if="entry.isCheckpoint" class="text-sm text-gray-600 dark:text-gray-400">
              ...
              <template v-else>
                ...
                <span class="font-semibold">{{
                  mode === "followers" ? entry.followers.totalCount : entry.following.totalCount
                }}</span>
              </template>
            </p>
          ...
          <button
            :class="[
              'p-2 rounded transition-all duration-200 ms-2',
              isAffectedByDelete(entry.timestamp)
                ? 'text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-900/30'
                : 'text-gray-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20',
            ]"
            @click.stop="handleDelete(entry.timestamp, $event)"
            @mouseenter="handleDeleteHover(entry.timestamp)"
            @mouseleave="handleDeleteHover(null)"
            :title="
              t('dashboard.details.history.delete_tooltip', {
                count: getAffectedTimestamps(entry.timestamp).length,
              })
            "
          >
            <Fa6SolidTrash class="w-4 h-4" />
          </button>
        </div>
        ...
        >
          ...
          <div v-else-if="snapshotDetails.has(entry.timestamp)" class="space-y-4">
            <UserChangesList
