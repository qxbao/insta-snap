    ...
    <div class="flex items-start gap-4">
      ...
      <div class="flex-1">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">
          {{ checking ? t('dashboard.main.migration.checking') : migrationComplete ? t('dashboard.main.migration.title_complete') : t('dashboard.main.migration.title_available') }}
        </h3>
        ...
        <div v-else-if="needsMig && !migrationComplete" class="space-y-3">
          ...
          >
            ...
            {{ migrating ? t('dashboard.main.migration.migrating_button') : t('dashboard.main.migration.start_button') }}
          </button>
        </div>
        ...
        <div v-else-if="migrationComplete && migrationStats" class="space-y-2">
          <div v-if="migrationStats.errors.length === 0">
            ...
            <div class="text-sm text-gray-600 dark:text-gray-400 space-y-1">
              <p>• {{ t('dashboard.main.migration.stats.users', { count: migrationStats.usersMetadata }) }}</p>
              <p>• {{ t('dashboard.main.migration.stats.snapshots', { count: migrationStats.snapshots }) }}</p>
              <p>• {{ t('dashboard.main.migration.stats.crons', { count: migrationStats.crons }) }}</p>
            </div>
          </div>
          <div v-else>
            ...
            <ul class="text-xs text-gray-600 dark:text-gray-400 space-y-1">
              <li v-for="(error, idx) in migrationStats.errors" :key="idx" class="flex items-start gap-2">
                <Fa6SolidXmark class="text-red-500 flex-shrink-0 mt-0.5" />
                <span>{{ error }}</span>
              </li>
            </ul>
          </div>
