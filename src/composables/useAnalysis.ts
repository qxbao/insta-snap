...
export function useAnalysis() {
  ...
  const analyzeSnapshot = async (userId: string, upToTimestamp: number) => {
    ...
    try {
      const followersSet = await database.getFullList(userId, upToTimestamp, true)
      const followingSet = await database.getFullList(userId, upToTimestamp, false)
      ...
      const allUserIds = new Set([...followersNotFollowedBack, ...followingNotFollowingBack])

      const users = await database.userMetadata.where("id").anyOf(Array.from(allUserIds)).toArray()
